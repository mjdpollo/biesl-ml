...
## 7. ECG-side HR / HRV analysis

The per-recording, per-phase HR / RMSSD / LF / HF / LF-HF tables and the
aggregate per-stressor medians have been moved to a standalone file:
**[`ECG-data-analysis.md`](ECG-data-analysis.md)** (independent of the BR
peak detector, so reusable across all three detector runs).
